if which program_name &> /dev/null; then
  ...
  if [ -d "/usr/cosmocc" ]; then
    /usr/cosmocc/bin/cosmocc bishop.c chess.c movePieces.c printBoard.c tower.c calculateMoves.c methods.c king.c queen.c checkmate.c knight.c pawn.c printMenu.c moveCounters.c -o chess
  else
